...
HRESULT CLeft_Message::Init(void)
{
	...
	//m_nTextureIdx = pTexure->Regist("data\\left.png");//登録
		...
		{*/
			...
			while (pObj != nullptr)
			{
				...
				if (CObject::PLAYER == pObj->GetType())
				{
					...
					nCounterAnimLeft++; //カウンターを加算
					if (nCounterAnimLeft >= 2)
					{
						nCounterAnimLeft = 0;//カウンターを初期値に戻す
						nPatternAnimLeft++;//パターンNo.を更新する
						if (nPatternAnimLeft >= 2)
						{
							nPatternAnimLeft = 0;//パターンNo.を初期値に戻す
						}
...
{
	CLeft_Message* pLeft_Message = new CLeft_Message(1);//Left_Message生成
	pLeft_Message->Init();
